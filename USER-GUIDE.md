...
“学期历史回查”按所选日期范围（最多一年）重新读取 Attendance 及已配置的 Gmail、Moodle、Ed 来源，复用 OCR 缓存。回查只读，不会补签，结果与日常待签到队列分开；关闭设置页后仍在后台运行，完成后可下载。网站无法访问、分页未读完或任务中断时会标为不完整；未知状态不代表缺勤。浏览器关闭后需重新回查。
