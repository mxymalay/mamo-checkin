...
双击包内的 **安装Mac识别服务.command**，按提示完成安装。需要 Python 3；若系统要求安装开发者命令行工具，请先完成安装。
...
助手每 5 秒检查一次安装状态。看到 **“安装成功 · 刷新并继续”** 后点击按钮，不必自己查找终端提示。

接着按页面顺序完成：

**填写学校姓名和邮箱 → 登录签到系统 → 点击检测课程 → 选择邮件/Moodle 来源 → 保存。**

全部完成后才会显示完整页面。确认来源已登录后，可开启自动运行或立即检查。

## 升级旧版本

1. 下载新版并解压，用新版 **extension** 内容覆盖原先加载的扩展目录。
2. 若本机组件有更新，重新运行 **安装Mac识别服务.command**。
