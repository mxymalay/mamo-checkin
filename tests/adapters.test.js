...
const config={email:'student@example.edu',name:'Example Student',courses:['FIT5120','FIT5122'],senders:{FIT5120:'lms@example.edu',FIT5122:'teacher@example.edu'}};
...
test('thread list uses configured course subject keywords without hardcoded attendance wording',()=>{
 ...
 const doc=dom(`<button aria-label="Google Account: Person (student@example.edu)"></button><main role="main"><table role="grid"><tr role="row"><td><span email="teacher@example.edu"></span><span data-legacy-thread-id="generic" data-legacy-last-message-id="latest">COMP90024 Weekly token</span></td></tr></table></main>`);
...
test('overlapping subject rules never choose the first matching course',()=>{
 ...
 const doc=dom(`<button aria-label="Google Account: Person (student@example.edu)"></button><main role="main"><table role="grid"><tr role="row"><td><span email="teacher@example.edu"></span><span data-legacy-thread-id="ambiguous" data-legacy-last-message-id="latest">Weekly code</span></td></tr></table></main>`);
...
test('final Gmail extraction waits for every allowed message body after navigation is ready',()=>{
 ...
 const doc=dom(`<button aria-label="Google Account: Person (student@example.edu)"></button><main><h2>${subject}</h2>${message('old','<div class="a3s"><p>Attendance old</p></div>')}${message('new','<div class="a3s" hidden><p>Attendance new</p></div>')}</main>`);
...
test('a course summary can contain only a table image without an attendance caption',()=>{
 const doc=dom('<button aria-label="Google Account: Person (student@example.edu)"></button><main><h2>FIT5120 Week 6 Summary</h2><div data-legacy-message-id="image-only"><span email="lms@example.edu"></span><span class="g3" title="4 Sept 2026, 22:08"></span><div class="a3s"><img src="https://mail.google.com/table.png"></div></div></main>');
