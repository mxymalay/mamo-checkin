...
export async function localService({onProgress=async()=>{},timeoutMs=35000}={}){
  ...
  return {call(payload){
    const job=queue.catch(()=>{}).then(async()=>{
      ...
      if(payload.op==='ping'&&result.engine==='Tesseract'&&!(result.ocrRevision>=4)){result={...result,binaryReady:false,healthError:'请使用最新的 Install Windows OCR.exe 安装改进后的 Windows 识别服务。'};}
