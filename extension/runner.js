...
export async function submitPending(state,adapter,now=Date.now()) {
  ...
  for(const r of state.records) {
    ...
    const candidates=[...new Set([r.code,...(r.codeCandidates||[])].filter(c=>/^[A-Z0-9]{5}$/.test(c||'')))];
    ...
    if(!submitted&&tried.length&&r.status!=='uncertain'){r.status='review';r.reason=`已尝试 ${tried.length} 个候选码均未被网站接受：${tried.join('、')}；请人工核对`;}
