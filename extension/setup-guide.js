...
export function createSetupGuide({doc=document,request,refresh,detect,checkHealth,reload,reset,skipOcr,windows=isWindows}){
 ...
 const $=id=>doc.getElementById(id);const identityBindings={},identitySubmit=$('setup-identity').querySelector('button[type=submit]');const syncIdentitySubmit=()=>{identitySubmit.hidden=!identityBindings.attendance?.verified;};const syncIdentityControls=()=>{const locked=identityAutoBusy>0;identityBindings.attendance?.setLocked?.(locked);identitySubmit.disabled=locked;$('setup-import-button').disabled=false;$('setup-reset').disabled=locked;};const beginIdentityAuto=()=>{identityAutoBusy++;syncIdentityControls();};const endIdentityAuto=()=>{identityAutoBusy=Math.max(0,identityAutoBusy-1);syncIdentityControls();};const scheduleIdentitySubmit=()=>doc.defaultView.setTimeout(syncIdentitySubmit,0);
 ...
 render();return {
  ...
  stopPendingReads(){identityAutoQueued=false;for(const binding of Object.values(identityBindings))binding.stop?.();},
