...
export function bindIdentityReader({input,button,status,request,onChange=()=>{},onVerified,onError=()=>{},buttonText='登录并读取姓名',verifiedButtonText='重新登录并读取姓名',doc=document}){
 ...
 },onVerified:async result=>{input.value=result.name;onChange();await onVerified?.(result.name);},onError,success:onVerified?'Attendance 姓名检测通过，已保存。':'已读取 Attendance 姓名，请确认后保存。'});
