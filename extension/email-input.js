...
export function installIdentityChecks({email,name,nameButton,nameStatus,request,onVerified,onChange=()=>{},doc=document}){
 ...
 name.closest('label').after(nameButton);if(nameActivity)nameButton.after(nameActivity);(nameActivity||nameButton).after(nameStatus);
 ...
 return bindEmailReader({input:email,button,status,request,onVerified,onChange,doc});
}
