...
export async function localService({onProgress=async()=>{},transient=false}={}){
  let closed=false;
  ...
  let stopOcr;const stopped=new Promise(resolve=>{stopOcr=resolve;});
  ...
  const callOffscreen=async payload=>{
    ...
    try{
      const result=await Promise.race([chrome.runtime.sendMessage({target:'ocr-offscreen',...payload}).catch(cause=>{const error=new Error('识别服务连接中断：'+(cause?.message||String(cause)));error.resetRequired=true;throw error;}),...(payload.op==='ocr'?[stopped.then(()=>{throw new Error('识别服务已关闭');})]:[]),new Promise((_,reject)=>{timer=setTimeout(()=>{const error=new Error(`${payload.op==='ocr'?'图片识别':'识别服务状态查询'}响应超时（${timeout/1000} 秒）`);error.resetRequired=true;reject(error);},timeout);})]);
  ...
  const reset=()=>exclusive(async()=>{if(closed)throw new Error('识别服务已关闭');await chrome.offscreen.closeDocument().catch(()=>{});if(closed)throw new Error('识别服务已关闭');await ensure();});
  ...
  return {call:async payload=>{
    ...
    if(payload.op==='ocr'){
      ...
      if(closed)throw new Error('识别服务已关闭');
      if(transient)return {...result,imageId:hash,cached:false};
  ...
  },close:async()=>{if(closed)return;closed=true;stopOcr();clearInterval(heartbeat);await exclusive(async()=>{clients--;if(clients===0)try{await callOffscreen({op:'release'});}finally{await chrome.offscreen.closeDocument();}});}};
}
